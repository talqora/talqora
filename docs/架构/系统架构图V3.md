...
flowchart TD
    ...
    %% ═══════════ ④ 业务层 ═══════════
    subgraph L4["④ 业务层 · 业务服务（Go）×N 副本 —— 仅内网<br/>（无本地可变状态,共享状态全外置 Redis/PG）"]
...
| HTTP 反代 | `/api /user /oauth /health` 直转业务层，流式转发大响应体（不整体缓冲）；反代前统一鉴权/限流/观测 | 反代零损耗（实测 login 19.7 vs 直连 19.5 rps） |
...
| 能力 | 设计 |
|---|---|
| 副本拓扑 | gateway ×N（有连接状态、可任意扩缩容，presence 登记 replica id）；business ×N（**无本地可变状态 + 共享状态全部外置**——presence/发号/通话态/幂等/已读全在 Redis/PG，进程内唯一可变状态仅为 socket.io 回滚引用） |
...
| 一致性 | 见下"跨副本原子性审计清单"——**无状态 ≠ 多副本安全**，所有 read-modify-write 必须原子化 |

**跨副本原子性审计清单**（无状态多副本的正确性前提，逐项核对过源码）：

| 共享状态操作 | 当前实现 | 多副本安全 | V3 要求 |
|---|---|---|---|
| 已读单调推进 | `updateMany WHERE lastReadSeq < uptoSeq`（`server/src/services/read.ts:22-26`，DB 条件更新） | ✅ 原子 | 保持 |
| seq 发号 | V3 改 Redis `INCR` | ✅ 原子（跳号一致性见 §4.1 checkpoint） | 保持 |
| 幂等去重 | DB 唯一约束 | ✅ 原子 | 保持 |
| presence 登记 | `multi`(ZADD+HSET)（`presence.ts:38-42`） | ✅ 基本原子 | 保持 |
| **通话忙线裁决** | `tryCreateSession` = GET 忙线 → SET 会话+索引（`callSession.ts:49-73`），**read-modify-write 非原子** | ❌ 两副本并发 call:start 可双接 | **改 `SET NX` 占位 + Lua 原子判定**（P3 前必须修） |
...
| **V3-P4 全链路分布式** | business ×N 副本 + 定向下行 + gRPC health；**先完成 §4.3 跨副本原子性审计**（通话忙线裁决改原子操作） | 多副本下全场景回归 + 副本扩缩容演练 |
...
| 风险 | 对策 |
|---|---|
| **跨副本 read-modify-write 竞态（已发现：通话忙线裁决双接）** | 多副本上线前逐项原子化：忙线改 `SET NX`+Lua；新增状态操作一律走原子原语（见 §4.3 审计清单） |
