...
|---|---|---|---|
| 已读单调推进 | `updateMany WHERE lastReadSeq < uptoSeq`（`read.ts:22-26`，DB 条件更新） | ✅ 原子 | 保持 |
| seq 发号 | V3 改 Redis `INCR` | ✅ 原子（跳号一致性见 §4.1 checkpoint） | 保持 |
| 幂等去重 | DB 唯一约束 | ✅ 原子 | 保持 |
| presence 登记 | `multi`(ZADD+HSET)（`presence.ts:38-42`） | ✅ 基本原子 | 保持 |
| **通话忙线裁决** | `tryCreateSession` = GET 忙线 → SET 会话+索引（`callSession.ts:56-72`），**read-modify-write 非原子** | ❌ 两副本并发 call:start 可双接 | **改 `SET NX` 占位 + Lua 原子判定**（P3 前必须修） |
